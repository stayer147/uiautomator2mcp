...
def test_unlock_uses_upstream_order_and_attempts_diagonal_swipe_fallback() -> None:
    device = Mock()
    ...
    device.swipe.assert_called_once_with(0.1, 0.9, 0.9, 0.1)
    assert result == 'Device unlocked and keyguard overlay cleared.'


def test_unlock_skips_fallback_swipe_when_overlay_is_not_present() -> None:
    device = Mock()
